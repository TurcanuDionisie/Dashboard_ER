...
since_inception = True    #True e False per decidere data di inizio calcolo per i singoli fondi
...
if since_inception == True:
    ...
    for isin in names_isin_dict.keys():
        ...
        #filtro per data inizio
        ret_quota_netta = quota_netta.pct_change()[1:][dettaglio_fondo].dropna()
        ret_quota_lorda = quota_lorda.pct_change()[1:][dettaglio_fondo].dropna()
        
        if dettaglio_fondo in nome_fondi_lordo.index:
            ret_bmk = bmk.pct_change()[1:][nome_fondi_lordo['serve per BMK'].loc[dettaglio_fondo]].dropna()
        ...
        if dettaglio_fondo in nome_fondi_netto.index:
            #NETTA
            ret_categoria = cat_morningstar.pct_change()[1:][nome_fondi_netto['serve per CAT M*'].loc[dettaglio_fondo]].dropna()
        ...
        if (dettaglio_fondo in (nome_fondi_lordo.index)):
            cum_quota_lorda = cum_quota_lorda.dropna()
            cum_bmk = cum_bmk.dropna()
            er_fund_lordo = cum_quota_lorda - cum_bmk
            er_fund_lordo = er_fund_lordo.dropna()
            
        if (dettaglio_fondo in (nome_fondi_netto.index)):
            cum_quota_netta = cum_quota_netta.dropna()
            cum_categoria = cum_categoria.dropna()
            er_fund_netto = cum_quota_netta - cum_categoria
            er_fund_netto = er_fund_netto.dropna()
    ...
    dictionary = {}
    dictionary.update(result_dict)
    del dictionary['GAM JUNIOR']
    del dictionary['GAM MAXI BOND']
    # Function to create a sample Plotly figure and return it as an HTML div
    def create_plot(i):
        er_graph = go.Figure()
        er_graph.add_trace(go.Scatter(x=dictionary[i].index, y=dictionary[i]['Net Perf vs Cat M*'], mode='lines', name='Net Perf vs Cat M*', line=dict(color='lightsteelblue'), hovertemplate='(%{x}, %{y:.3%})'))
